...
class ContainerProxy(
  ...
  storeActivation: (TransactionId, WhiskActivation, UUID) => Future[Any],
      ...
      container
        .andThen {
          ...
          case Failure(t) =>
            ...
            storeActivation(transid, activation, job.msg.user.namespace.uuid)
        }
        .flatMap { container =>
  ...
  def initializeAndRun(container: Container, job: Run)(implicit tid: TransactionId): Future[WhiskActivation] = {
    ...
    activationWithLogs.map(_.fold(_.activation, identity)).foreach(storeActivation(tid, _, job.msg.user.namespace.uuid))
...
object ContainerProxy {
  def props(
    ...
    store: (TransactionId, WhiskActivation, UUID) => Future[Any],
    collectLogs: (TransactionId, Identity, WhiskActivation, Container, ExecutableWhiskAction) => Future[ActivationLogs],
    instance: InvokerInstanceId,
    poolConfig: ContainerPoolConfig,
