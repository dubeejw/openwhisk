                ...
                respondWithHeaders(headers) {
                    mediaType.toString.split("/") match {
                        case Array(mainType, subType) =>
                            complete(code, HttpEntity(ContentType(MediaType.customWithFixedCharset(mainType, subType, HttpCharsets.`UTF-8`)), data))
                    }
                }
    ...
    private def extractEntityAndProcessRequest(
        ...
        provide(action.annotations.asBool("raw-http").exists(identity)) { isRawHttpAction =>
            httpEntity match {
                ...
                case HttpEntity.Strict(ContentTypes.`application/json`, data) if !isRawHttpAction =>
