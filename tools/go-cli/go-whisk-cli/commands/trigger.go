...
var triggerFireCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if len(flags.common.param) > 0 {
            parameters, err := parseParameters2(flags.common.param)
            if err != nil {
...
var triggerCreateCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        parameters, err := parseParameters3(flags.common.param)
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if retTrigger != nil && retTrigger.Annotations != nil {
            for i := 0; i < len(retTrigger.Annotations); i++ {
                switch key := retTrigger.Annotations[i]["keya"].(type) {
                    case string: {
                        if key == "feed" {
...
func deleteFeed (triggerName string, FullFeedName string, parameters []string) error {
    var originalParams = flags.common.param
//changes?blocking=true&result=false
//url = 'https://%(apibase)s/namespaces/%(namespace)s/actions/%(name)s?blocking=%(blocking)s&result=%(result)s' % {

    ///dummyCloudantPackage/changes?blocking=true&result=false

    //dummyCloudantTrigger-1466181193417?blocking=true

    // Invoke the feed action to delete the feed
    feedArgs := []string {FullFeedName}
    flags.common.param = parameters
    flags.common.blocking = true
    err := actionInvokeCmd.RunE(nil, feedArgs)
