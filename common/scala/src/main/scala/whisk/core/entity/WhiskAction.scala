      ...
      doc.exec match {
        case exec @ CodeExecAsAttachment(_, Inline(code), _) =>
          ...
          for (i1 <- super.put(db, newDoc);
               i2 <- attach[A](db, newDoc, i1, manifest.attachmentName, manifest.attachmentType, stream, Some {
                 a: WhiskAction =>
    ...
    fa.flatMap { action =>
      action.exec match {
        case exec @ CodeExecAsAttachment(_, Attached(attachmentName, _), _) =>
          ...
          getAttachment[A](db, action, action.docinfo, attachmentName, b64s, Some { a: WhiskAction =>
            b64s.close()
