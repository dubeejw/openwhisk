...
import whisk.core.database.PostProcess2.PostProcessEntity2


package object PostProcess2 {
  type PostProcessEntity2[A] = A => A
}
...
trait DocumentFactory[W] extends MultipleReadersSingleWriterCache[W, DocInfo] {
  ...
  def attach[Wsuper >: W](
    ...
    bytes: InputStream,
    postProcess: Option[PostProcessEntity2[W]] = None)(implicit transid: TransactionId, notifier: Option[CacheChangeNotification]): Future[DocInfo] = {
    ...
    Try {
      ...
      // run callback that inlines attachment code for entity

      cacheUpdate(postProcess map { _(entity) } getOrElse entity, key, db.attach(doc, attachmentName, contentType, src) map { docinfo =>
        entity match {
          // if doc has a revision id, update it with new version
          case w: DocumentRevisionProvider => w.revision[W](docinfo.rev)
        }
  ...
  def getAttachment[Wsuper >: W](db: ArtifactStore[Wsuper],
                                 ...
                                 postProcess: Option[PostProcessEntity2[W]] = None)(implicit transid: TransactionId, mw: Manifest[W]): Future[W] = {
    ...
    Try {
      ...
      db.readAttachment[IOResult](doc, attachmentName, sink).map {
        case (_, r) =>
          ...
          cacheUpdate(e, key, Future.successful(doc))
          e
      }
